...
#define PROP_VISIT_GETTER_SETTER(_type, _name)                               \
    static void t8103_get_##_name(Object *obj, Visitor *v, const char *name, \
                                  void *opaque, Error **errp)                \
    {                                                                        \
        _type##_t value;                                                     \
                                                                             \
        value = APPLE_T8103(obj)->_name;                                     \
        visit_type_##_type(v, name, &value, errp);                           \
    }                                                                        \
                                                                             \
    static void t8103_set_##_name(Object *obj, Visitor *v, const char *name, \
                                  void *opaque, Error **errp)                \
    {                                                                        \
        visit_type_##_type(v, name, &APPLE_T8103(obj)->_name, errp);         \
    }

#define PROP_STR_GETTER_SETTER(_name)                             \
    static char *t8103_get_##_name(Object *obj, Error **errp)     \
    {                                                             \
        return g_strdup(APPLE_T8103(obj)->_name);                 \
    }                                                             \
                                                                  \
    static void t8103_set_##_name(Object *obj, const char *value, \
                                  Error **errp)                   \
    {                                                             \
        AppleT8103MachineState *t8103;                            \
                                                                  \
        t8103 = APPLE_T8103(obj);                                 \
        g_free(t8103->_name);                                     \
        t8103->_name = g_strdup(value);                           \
    }

#define PROP_GETTER_SETTER(_type, _name)                                  \
    static void t8103_set_##_name(Object *obj, _type value, Error **errp) \
    {                                                                     \
        APPLE_T8103(obj)->_name = value;                                  \
    }                                                                     \
                                                                          \
    static _type t8103_get_##_name(Object *obj, Error **errp)             \
    {                                                                     \
        return APPLE_T8103(obj)->_name;                                   \
    }
...
static uint32_t t8103_real_cpu_count(AppleT8103MachineState *t8103)
{
    return MACHINE(t8103)->smp.cpus - (t8103->sep_fw_filename != NULL);
}

static void t8103_start_cpus(AppleT8103MachineState *t8103, uint64_t cpu_mask)
{
    uint32_t i;

    for (i = 0; i < t8103_real_cpu_count(t8103); ++i) {
        if ((cpu_mask & BIT_ULL(i)) != 0) {
            apple_m1_set_on(t8103->cpus[i]);
        }
    }
}

static void t8103_create_s3c_uart(const AppleT8103MachineState *t8103,
                                  uint32_t port, Chardev *chr)
{
    ...
    AppleDTProp *prop;
    hwaddr *uart_offset;
    AppleDTNode *child = apple_dt_get_node(t8103->device_tree, "arm-io/uart0");
    char name[32];
    ...
    base = t8103->armio_base + uart_offset[0] + uart_offset[1] * port;
    ...
    dev = apple_uart_create(
        base, 15, 0, chr,
        qdev_get_gpio_in(DEVICE(t8103->aic), lduw_le_p(prop->data) + port));
    assert_nonnull(dev);
    dev->id = g_strdup(name);
}

static void t8103_patch_kernel(MachoHeader64 *header, uint32_t build_version)
{
    ck_patch_kernel(header);
}

static bool t8103_check_panic(AppleT8103MachineState *t8103)
{
    AppleEmbeddedPanicHeader *panic_info;
    bool ret;

    if (t8103->panic_size == 0) {
        return false;
    }

    panic_info = g_malloc0(t8103->panic_size);

    address_space_rw(&address_space_memory, t8103->panic_base,
                     MEMTXATTRS_UNSPECIFIED, panic_info, t8103->panic_size,
                     false);
    address_space_set(&address_space_memory, t8103->panic_base, 0,
                      t8103->panic_size, MEMTXATTRS_UNSPECIFIED);
...
static void get_kaslr_slides(AppleT8103MachineState *t8103,
...
static void t8103_load_kernelcache(AppleT8103MachineState *t8103,
                                   const char *cmdline, CarveoutAllocator *ca)
{
    MachineState *machine = MACHINE(t8103);
    vaddr kc_base;
    vaddr kc_end;
    vaddr ro_lower;
    vaddr ro_upper;
    ...
    hwaddr mem_size;
    hwaddr phys_ptr;
    AppleBootInfo *info = &t8103->boot_info;

    apple_boot_get_kc_bounds(t8103->kernel, NULL, &kc_base, &kc_end, &ro_lower,
                             &ro_upper);

    get_kaslr_slides(t8103, &g_phys_slide, &g_virt_slide);
    ...
    address_space_rw(&address_space_memory, info->trustcache_addr,
                     MEMTXATTRS_UNSPECIFIED, t8103->trustcache,
                     info->trustcache_size, true);

    info->kern_entry = apple_boot_load_macho(
        t8103->kernel, &address_space_memory,
        apple_dt_get_node(t8103->device_tree, "/chosen/memory-map"),
    ...
    if (t8103->sep_fw_filename != NULL) {
        AppleSEPState *sep = APPLE_SEP(
            object_property_get_link(OBJECT(t8103), "sep", &error_fatal));
        sep->sep_fw_addr = info->sep_fw_addr;
        if (!g_file_get_contents(t8103->sep_fw_filename, &sep->fw_data,
                                 &sep->sep_fw_size, NULL)) {
            error_setg(&error_fatal, "Failed to read SEP Firmware from `%s`",
                       t8103->sep_fw_filename);
            return;
        }
    }
    ...
    apple_boot_finalise_dt(t8103->device_tree, &address_space_memory, info);

    mem_size = carveout_alloc_finalise(ca);
    info_report("Memory Size: 0x" HWADDR_FMT_plx, mem_size);

    apple_boot_setup_bootargs(
        t8103->build_version, &address_space_memory, info->kern_boot_args_addr,
        g_virt_base, g_phys_base, mem_size, info->top_of_kernel_data_pa,
        apple_dt_va, info->device_tree_size, &t8103->video_args, cmdline,
        machine->ram_size);
    g_virt_base = kc_base;

    for (int i = 0; i < AMCC_PLANE_COUNT; ++i) {
        AMCC_WREG32(t8103, AMCC_PLANE_LOWER_LIMIT(i),
                    (info->trustcache_addr - info->dram_base) >> 14);
        AMCC_WREG32(t8103, AMCC_PLANE_UPPER_LIMIT(i),
                    (vtop_slid(ro_upper) - info->dram_base - 1) >> 14);
        AMCC_WREG32(t8103, AMCC_PLANE_LOCK(i), 1);
        AMCC_WREG32(t8103, AMCC_PLANE_TZ0_BASE(i),
                    (info->tz0_addr - info->dram_base) >> 12);
        AMCC_WREG32(t8103, AMCC_PLANE_TZ0_END(i),
                    ((info->tz0_addr + info->tz0_size - 1) - info->dram_base) >>
                        12);
        AMCC_WREG32(t8103, AMCC_PLANE_TZ0_LOCK(i), 1);
        AMCC_WREG32(t8103, AMCC_PLANE_BLK_MCC_CHANNEL_DEC(i), 0x2F);
    }
}

static void t8103_rtkit_seg_prop_setup(AppleDTNode *iop_nub, hwaddr base,
                                       uint32_t size)
{
...
static void t8103_rtkit_mem_setup(AppleT8103MachineState *t8103,
                                  ...
                                  const char *nub_name, uint32_t size)
{
    ...
    t8103_rtkit_seg_prop_setup(iop_nub, carveout_alloc_mem(ca, size), size);
}

static void t8103_memory_setup(AppleT8103MachineState *t8103)
{
    ...
    apple_dt_unfinalise(t8103->device_tree);

    carveout_memory_map =
        apple_dt_get_node(t8103->device_tree, "/chosen/carveout-memory-map");

    machine = MACHINE(t8103);
    info = &t8103->boot_info;
    memory_map = apple_dt_get_node(t8103->device_tree, "/chosen/memory-map");

    if (t8103_check_panic(t8103)) {
    ...
    t8103_rtkit_mem_setup(t8103, ca, "sio", "iop-sio-nub", SIO_SIZE);
    t8103_rtkit_mem_setup(t8103, ca, "ans", "iop-ans-nub", ANS_SIZE);

    if (t8103->sep_rom_filename) {
        if (!g_file_get_contents(t8103->sep_rom_filename, &seprom, &fsize,
                                 NULL)) {
            error_setg(&error_fatal, "Could not load data from file '%s'",
                       t8103->sep_rom_filename);
            return;
        }
    ...
    info_report("Boot Mode: %u", t8103->boot_info.boot_mode);
    switch (t8103->boot_info.boot_mode) {
    case kAppleBootModeEnterRecovery:
        auto_boot = false;
        env_unset(nvram, "auto-boot");
        t8103->boot_info.boot_mode = kAppleBootModeAuto;
        break;
    ...
    if (t8103->boot_info.non_cold_boot &&
        (!t8103->boot_info.had_autoboot && have_autoboot && auto_boot)) {
        fprintf(stdout,
    ...
    t8103->boot_info.had_autoboot = have_autoboot;
    t8103->boot_info.non_cold_boot = true;
    ...
    AppleDTNode *chosen = apple_dt_get_node(t8103->device_tree, "chosen");
    ...
    t8103->video_args.base_addr = carveout_alloc_mem(ca, DISPLAY_SIZE);
    adp_v4_update_vram_mapping(
        APPLE_DISPLAY_PIPE_V4(
            object_property_get_link(OBJECT(t8103), "disp0", &error_abort)),
        machine->ram, t8103->video_args.base_addr - DRAM_BASE, DISPLAY_SIZE);

    if (t8103->securerom_filename != NULL) {
        address_space_rw(&address_space_memory, SROM_BASE,
                         MEMTXATTRS_UNSPECIFIED, t8103->securerom,
                         t8103->securerom_size, 1);
        return;
    }
    ...
    apple_dt_set_prop(t8103->device_tree, "compatible", 28,
    ...
    AppleDTNode *pram = apple_dt_get_node(t8103->device_tree, "pram");
    if (pram != NULL) {
        ...
        t8103->panic_base = panic_reg[0];
        t8103->panic_size = panic_reg[1];
    }

    AppleDTNode *vram = apple_dt_get_node(t8103->device_tree, "vram");
    if (vram != NULL) {
        uint64_t vram_reg[2];
        vram_reg[0] = t8103->video_args.base_addr;
        vram_reg[1] = DISPLAY_SIZE;
        apple_dt_set_prop(vram, "reg", sizeof(vram_reg), &vram_reg);
    }
    // adjust framebuffer for verbose display.
    t8103->video_args.base_addr += adp_v4_get_fb_off(APPLE_DISPLAY_PIPE_V4(
        object_property_get_link(OBJECT(t8103), "disp0", &error_abort)));
    ...
    apple_boot_populate_dt(t8103->device_tree, info, auto_boot);

    switch (hdr->file_type) {
    case MH_EXECUTE:
    case MH_FILESET:
        t8103_load_kernelcache(t8103, cmdline, ca);
        g_free(cmdline);
        break;
    default:
...
{
    AppleT8103MachineState *t8103 = APPLE_T8103(qdev_get_machine());
    AppleSEPState *sep;
    hwaddr base = (hwaddr)opaque;
    ...
    case 0x3D2BC004: // Current Secure Mode
        sep = APPLE_SEP(object_property_get_link(OBJECT(t8103), "sep", NULL));
    ...
    case 0x3D2BC410: // Raw Board ID and Epoch, bit 30 should remain unset
        return ((t8103->board_id >> 5) & 0x7) | ((security_epoch & 0x7f) << 5) |
    ...
    case 0x3D2BC030: // Chip Revision
        return ((t8103->chip_revision & 0x7) << 6) |
               (((t8103->chip_revision & 0x70) >> 4) << 5) | (0 << 1);
    case 0x3D2BC300: // ECID LOW T8103
        return extract64(t8103->ecid, 0, 32);
    case 0x3D2BC304: // ECID HIGH T8103
        return extract64(t8103->ecid, 32, 32);
    case 0x3D2BC30C: {
        // T8103 SEP Chip Revision
...
static void pmgr_reg_write(void *opaque, hwaddr addr, uint64_t data,
                           unsigned size)
{
    AppleT8103MachineState *t8103 = opaque;
    AppleSEPState *sep;
    ...
    case 0x80C00: // SEP_PS
        ...
        if (sep != NULL) {
            if (data & BIT32(31)) {
                apple_m1_reset(APPLE_M1(sep->cpu));
            } else if (data & BIT32(10)) {
                apple_m1_set_off(APPLE_M1(sep->cpu));
            } else {
                apple_m1_set_on(APPLE_M1(sep->cpu));
            }
        }
        break;
    default:
    ...
    if (size == 8) {
        stq_le_p(t8103->pmgr_reg + addr, data);
    } else {
        stl_le_p(t8103->pmgr_reg + addr, data);
    }
}
...
static uint64_t pmgr_reg_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    switch (addr) {
    case 0xF0010: // AppleT8103PMGR::commonSramCheck
        result = 0x5000;
        break;
    default:
        result = size == 8 ? ldq_le_p(t8103->pmgr_reg + addr) :
                             ldl_le_p(t8103->pmgr_reg + addr);
        break;
    }
                           ...
                           unsigned size)
{
    ...
    if ((AMCC_RREG32(t8103, AMCC_PLANE_LOCK(addr / AMCC_PLANE_STRIDE)) != 0 &&
        ...
        (AMCC_RREG32(t8103, AMCC_PLANE_TZ0_LOCK(addr / AMCC_PLANE_STRIDE)) !=
             0 &&
    ...
    stl_le_p(t8103->amcc_reg + addr, data);
}
...
static uint64_t amcc_reg_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    result = ldl_le_p(t8103->amcc_reg + addr);
...
static void t8103_cluster_setup(AppleT8103MachineState *t8103)
{
    for (uint32_t i = 0; i < M1_MAX_CLUSTER; ++i) {
        char *name = NULL;

        name = g_strdup_printf("cluster%u", i);
        object_initialize_child(OBJECT(t8103), name, &t8103->clusters[i],
                                TYPE_APPLE_M1_CLUSTER);
        g_free(name);
        qdev_prop_set_uint32(DEVICE(&t8103->clusters[i]), "cluster-id", i);
    }
}

static void t8103_cluster_realize(AppleT8103MachineState *t8103)
{
    for (int i = 0; i < M1_MAX_CLUSTER; ++i) {
        qdev_realize(DEVICE(&t8103->clusters[i]), NULL, &error_fatal);
    }
}

static void t8103_cpu_setup(AppleT8103MachineState *t8103)
{
    unsigned int i;
    AppleDTNode *root;
    GList *iter;
    ...
    t8103_cluster_setup(t8103);

    root = apple_dt_get_node(t8103->device_tree, "cpus");
    assert_nonnull(root);
    ...
    for (iter = root->children, i = 0; iter; iter = next, ++i) {
        ...
        if (i >= t8103_real_cpu_count(t8103)) {
            apple_dt_del_node(root, node);
            continue;
        }

        t8103->cpus[i] = apple_m1_from_node(node);
        cluster_id = t8103->cpus[i]->cluster_id;

        object_property_add_child(OBJECT(&t8103->clusters[cluster_id]),
                                  DEVICE(t8103->cpus[i])->id,
                                  OBJECT(t8103->cpus[i]));
        qdev_realize(DEVICE(t8103->cpus[i]), NULL, &error_fatal);
    }
    t8103_cluster_realize(t8103);
}

static void t8103_create_aic(AppleT8103MachineState *t8103)
{
    uint32_t i;
    hwaddr *reg;
    AppleDTProp *prop;
    AppleDTNode *soc = apple_dt_get_node(t8103->device_tree, "arm-io");
    AppleDTNode *child;
    ...
    t8103->aic = apple_aic_create(t8103_real_cpu_count(t8103), child, timebase);
    object_property_add_child(OBJECT(t8103), "aic", OBJECT(t8103->aic));
    assert_nonnull(t8103->aic);
    sysbus_realize(t8103->aic, &error_fatal);
    ...
    for (i = 0; i < t8103_real_cpu_count(t8103); ++i) {
        memory_region_add_subregion_overlap(
            &t8103->cpus[i]->memory, t8103->armio_base + reg[0],
            sysbus_mmio_get_region(t8103->aic, i), 0);
        sysbus_connect_irq(
            t8103->aic, i,
            qdev_get_gpio_in(DEVICE(t8103->cpus[i]), ARM_CPU_IRQ));
    }
}

static void t8103_pmgr_setup(AppleT8103MachineState *t8103)
{
    uint64_t *reg;
    uint32_t i;
    char name[32];
    ...
    for (i = 0; i < prop->len / 8; i += 2) {
        ...
        if (i > 0) {
            ...
            memory_region_init_io(mem, OBJECT(t8103), &pmgr_unk_reg_ops,
                                  (void *)reg[i], name, reg[i + 1]);
        } else {
            memory_region_init_io(mem, OBJECT(t8103), &pmgr_reg_ops, t8103,
                                  "pmgr-reg", reg[i + 1]);
        }
        memory_region_add_subregion(get_system_memory(),
                                    reg[i] + reg[i + 1] < t8103->armio_size ?
                                        t8103->armio_base + reg[i] :
                                        reg[i],
                                    mem);
    }
    ...
    memory_region_add_subregion(get_system_memory(),
                                t8103->armio_base + 0x3BC00000, mem);

    apple_dt_set_prop(child, "voltage-states0", sizeof(t8103_voltage_states0),
                      t8103_voltage_states0);
    apple_dt_set_prop(child, "voltage-states1", sizeof(t8103_voltage_states1),
                      t8103_voltage_states1);
    apple_dt_set_prop(child, "voltage-states2", sizeof(t8103_voltage_states2),
                      t8103_voltage_states2);
    apple_dt_set_prop(child, "voltage-states5", sizeof(t8103_voltage_states5),
                      t8103_voltage_states5);
    apple_dt_set_prop(child, "voltage-states8", sizeof(t8103_voltage_states8),
                      t8103_voltage_states8);
    apple_dt_set_prop(child, "voltage-states9", sizeof(t8103_voltage_states9),
                      t8103_voltage_states9);
    apple_dt_set_prop(child, "voltage-states10", sizeof(t8103_voltage_states10),
                      t8103_voltage_states10);
    apple_dt_set_prop(child, "voltage-states11", sizeof(t8103_voltage_states11),
    ...
    apple_dt_set_prop(child, "voltage-states2-sram",
                      sizeof(t8103_voltage_states2_sram),
                      t8103_voltage_states2_sram);
    ...
    apple_dt_set_prop(child, "voltage-states8-sram",
                      sizeof(t8103_voltage_states8_sram),
                      t8103_voltage_states8_sram);
    ...
    apple_dt_set_prop(child, "bridge-settings-0",
                      sizeof(t8103_bridge_settings0), t8103_bridge_settings0);
    apple_dt_set_prop(child, "bridge-settings-1",
                      sizeof(t8103_bridge_settings1), t8103_bridge_settings1);
    apple_dt_set_prop(child, "bridge-settings-2",
                      sizeof(t8103_bridge_settings2), t8103_bridge_settings2);
    apple_dt_set_prop(child, "bridge-settings-3",
                      sizeof(t8103_bridge_settings3), t8103_bridge_settings3);
    apple_dt_set_prop(child, "bridge-settings-4",
                      sizeof(t8103_bridge_settings4), t8103_bridge_settings4);
    apple_dt_set_prop(child, "bridge-settings-5",
                      sizeof(t8103_bridge_settings5), t8103_bridge_settings5);
    apple_dt_set_prop(child, "bridge-settings-6",
                      sizeof(t8103_bridge_settings6), t8103_bridge_settings6);
    apple_dt_set_prop(child, "bridge-settings-7",
                      sizeof(t8103_bridge_settings7), t8103_bridge_settings7);
    apple_dt_set_prop(child, "bridge-settings-8",
                      sizeof(t8103_bridge_settings8), t8103_bridge_settings8);
    apple_dt_set_prop(child, "bridge-settings-12",
                      sizeof(t8103_bridge_settings12), t8103_bridge_settings12);
    apple_dt_set_prop(child, "bridge-settings-13",
                      sizeof(t8103_bridge_settings13), t8103_bridge_settings13);
    apple_dt_set_prop(child, "bridge-settings-14",
                      sizeof(t8103_bridge_settings14), t8103_bridge_settings14);
    apple_dt_set_prop(child, "bridge-settings-15",
                      sizeof(t8103_bridge_settings15), t8103_bridge_settings15);
    apple_dt_set_prop(child, "bridge-settings-16",
                      sizeof(t8103_bridge_settings16), t8103_bridge_settings16);
    apple_dt_set_prop(child, "bridge-settings-17",
                      sizeof(t8103_bridge_settings17), t8103_bridge_settings17);
}

static void t8103_amcc_setup(AppleT8103MachineState *t8103)
{
    AppleDTNode *child;

    child = apple_dt_get_node(t8103->device_tree, "chosen/lock-regs/amcc");
    if (child != NULL) {
    ...
    memory_region_init_io(&t8103->amcc, OBJECT(t8103), &amcc_reg_ops, t8103,
                          "amcc", AMCC_SIZE);
    memory_region_add_subregion(get_system_memory(), AMCC_BASE, &t8103->amcc);
}

static void t8103_create_dart(AppleT8103MachineState *t8103, const char *name,
                              bool absolute_mmio)
{
    ...
    assert_nonnull(dart);
    object_property_add_child(OBJECT(t8103), name, OBJECT(dart));
    ...
    for (i = 0; i < prop->len / 16; ++i) {
        sysbus_mmio_map(SYS_BUS_DEVICE(dart), i,
                        (absolute_mmio ? 0 : t8103->armio_base) + reg[i * 2]);
    }
    ...
    for (i = 0; i < prop->len / sizeof(uint32_t); ++i) {
        sysbus_connect_irq(SYS_BUS_DEVICE(dart), i,
                           qdev_get_gpio_in(DEVICE(t8103->aic), ints[i]));
    }

    sysbus_realize_and_unref(SYS_BUS_DEVICE(dart), &error_fatal);
}

static void t8103_create_sart(AppleT8103MachineState *t8103)
{
    uint64_t *reg;
    AppleDTNode *child;
    AppleDTProp *prop;
    ...
    child = apple_dt_get_node(t8103->device_tree, "arm-io/sart-ans");
    assert_nonnull(child);

    sart = apple_sart_from_node(child);
    assert_nonnull(sart);
    object_property_add_child(OBJECT(t8103), "sart-ans", OBJECT(sart));

    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    reg = (uint64_t *)prop->data;

    sysbus_mmio_map(sart, 0, t8103->armio_base + reg[0]);
    sysbus_realize_and_unref(sart, &error_fatal);
}

static void t8103_create_ans(AppleT8103MachineState *t8103)
{
    uint32_t i;
    uint32_t *ints;
    AppleDTProp *prop;
    ...
    t8103_create_sart(t8103);
    sart = SYS_BUS_DEVICE(
        object_property_get_link(OBJECT(t8103), "sart-ans", &error_fatal));

    // bridge0 and bridge1 don't exist in the t8103 device tree
    ...
    PCIDevice *pci_dev = PCI_DEVICE(
        object_property_get_link(OBJECT(t8103), "pcie.bridge0", &error_fatal));
    PCIBridge *pci_bridge = PCI_BRIDGE(pci_dev);
    PCIBus *sec_bus = pci_bridge_get_sec_bus(pci_bridge);
    apcie_host = APPLE_PCIE_HOST(
        object_property_get_link(OBJECT(t8103), "pcie.host", &error_fatal));
    ...
    object_property_add_child(OBJECT(t8103), "ans", OBJECT(ans));
    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    reg = (uint64_t *)prop->data;

    for (i = 0; i < 4; ++i) {
        sysbus_mmio_map(ans, i, t8103->armio_base + reg[i << 1]);
    }
...
static void t8103_create_baseband(AppleT8103MachineState *t8103)
{
    int i;
    uint32_t *ints;
    AppleDTProp *prop;
    ...
    AppleDTNode *child = apple_dt_get_node(t8103->device_tree, "baseband");
    ApplePCIEHost *apcie_host;
    ...
    ApplePCIEPort *port = APPLE_PCIE_PORT(
        object_property_get_link(OBJECT(t8103), "pcie.bridge3", &error_fatal));
    ...
    object_property_add_child(OBJECT(t8103), "baseband", OBJECT(baseband));
...
static void t8103_create_gpio(AppleT8103MachineState *t8103, const char *name)
{
    DeviceState *gpio = NULL;
    AppleDTProp *prop;
    uint64_t *reg;
    ...
    assert_nonnull(prop);
    reg = (uint64_t *)prop->data;
    sysbus_mmio_map(SYS_BUS_DEVICE(gpio), 0, t8103->armio_base + reg[0]);
    prop = apple_dt_get_prop(child, "interrupts");
    assert_nonnull(prop);
...
static void t8103_create_i2c(AppleT8103MachineState *t8103, const char *name)
{
    ...
    assert_nonnull(i2c);
    object_property_add_child(OBJECT(t8103), name, OBJECT(i2c));

    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    reg = (uint64_t *)prop->data;
    sysbus_mmio_map(i2c, 0, t8103->armio_base + reg[0]);
    prop = apple_dt_get_prop(child, "interrupts");
    assert_nonnull(prop);

    sysbus_connect_irq(
        i2c, 0, qdev_get_gpio_in(DEVICE(t8103->aic), *(uint32_t *)prop->data));

    sysbus_realize_and_unref(i2c, &error_fatal);
}

static void t8103_create_spi0(AppleT8103MachineState *t8103)
{
    ...
    object_property_add_child(OBJECT(t8103), "spi0", OBJECT(spi));

    sio = object_property_get_link(OBJECT(t8103), "sio", &error_fatal);
    object_property_add_const_link(OBJECT(spi), "sio", sio);
    sysbus_realize_and_unref(SYS_BUS_DEVICE(spi), &error_fatal);

    sysbus_mmio_map(SYS_BUS_DEVICE(spi), 0, t8103->armio_base + SPI0_BASE);

    sysbus_connect_irq(SYS_BUS_DEVICE(spi), 0,
                       qdev_get_gpio_in(DEVICE(t8103->aic), SPI0_IRQ));
    // The second sysbus IRQ is the cs line
    gpio =
        DEVICE(object_property_get_link(OBJECT(t8103), "gpio", &error_fatal));
    assert_nonnull(gpio);
    qdev_connect_gpio_out(gpio, GPIO_SPI0_CS,
                          qdev_get_gpio_in_named(spi, SSI_GPIO_CS, 0));
}

static void t8103_create_spi(AppleT8103MachineState *t8103, uint32_t port)
{
    ...
    assert_nonnull(spi);
    object_property_add_child(OBJECT(t8103), name, OBJECT(spi));
    ...
    // The second sysbus IRQ is the cs line
    sysbus_connect_irq(SYS_BUS_DEVICE(spi), 0,
                       qdev_get_gpio_in(DEVICE(t8103->aic), irq));
...
static void t8103_create_usb(AppleT8103MachineState *t8103)
{
    AppleDTNode *child = apple_dt_get_node(t8103->device_tree, "arm-io");
    ...
    dart = APPLE_DART(
        object_property_get_link(OBJECT(t8103), "dart-usb", &error_fatal));

    atc = qdev_new(TYPE_APPLE_TYPEC);
    object_property_add_child(OBJECT(t8103), "atc", OBJECT(atc));

    object_property_set_str(
        OBJECT(atc), "conn-type",
        qapi_enum_lookup(&USBTCPRemoteConnType_lookup, t8103->usb_conn_type),
        &error_fatal);
    if (t8103->usb_conn_addr != NULL) {
        object_property_set_str(OBJECT(atc), "conn-addr", t8103->usb_conn_addr,
                                &error_fatal);
    }
    object_property_set_uint(OBJECT(atc), "conn-port", t8103->usb_conn_port,
                             &error_fatal);
    ...
    sysbus_mmio_map(SYS_BUS_DEVICE(atc), 0,
                    t8103->armio_base + ((uint64_t *)prop->data)[0]);
    ...
    sysbus_connect_irq(SYS_BUS_DEVICE(atc), 4,
                       qdev_get_gpio_in(DEVICE(t8103->aic), DWC2_IRQ));
}

static void t8103_create_wdt(AppleT8103MachineState *t8103)
{
    uint32_t i;
    uint32_t *ints;
    AppleDTProp *prop;
    ...
    object_property_add_child(OBJECT(t8103), "wdt", OBJECT(wdt));
    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    reg = (uint64_t *)prop->data;

    sysbus_mmio_map(wdt, 0, t8103->armio_base + reg[0]);
    sysbus_mmio_map(wdt, 1, t8103->armio_base + reg[2]);
...
static void t8103_create_aes(AppleT8103MachineState *t8103)
{
    uint32_t *ints;
    AppleDTProp *prop;
    uint64_t *reg;
    ...
    aes = apple_aes_create(child, t8103->board_id);
    assert_nonnull(aes);

    object_property_add_child(OBJECT(t8103), "aes", OBJECT(aes));
    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    reg = (uint64_t *)prop->data;

    sysbus_mmio_map(aes, 0, t8103->armio_base + reg[0]);
    sysbus_mmio_map(aes, 1, t8103->armio_base + reg[2]);
    ...
    sysbus_connect_irq(aes, 0, qdev_get_gpio_in(DEVICE(t8103->aic), *ints));

    dart = APPLE_DART(
        object_property_get_link(OBJECT(t8103), "dart-sio", &error_fatal));
    assert_nonnull(dart);
...
static void t8103_create_spmi(AppleT8103MachineState *t8103, const char *name)
{
    ...
    assert_nonnull(spmi);
    object_property_add_child(OBJECT(t8103), name, OBJECT(spmi));
    ...
    sysbus_mmio_map(SYS_BUS_DEVICE(spmi), 0,
                    (t8103->armio_base + reg[2]) & ~(APPLE_SPMI_MMIO_SIZE - 1));
    ...
    sysbus_connect_irq(SYS_BUS_DEVICE(spmi), 0,
                       qdev_get_gpio_in(DEVICE(t8103->aic), ints[1]));

    sysbus_realize_and_unref(SYS_BUS_DEVICE(spmi), &error_fatal);
}

static void t8103_create_pmu(AppleT8103MachineState *t8103, const char *parent,
                             const char *name)
{
    ...
    spmi = APPLE_SPMI(
        object_property_get_link(OBJECT(t8103), parent, &error_fatal));
    assert_nonnull(spmi);
    ...
    assert_nonnull(pmu);
    object_property_add_child(OBJECT(t8103), name, OBJECT(pmu));
...
static void t8103_create_baseband_spmi(AppleT8103MachineState *t8103,
                                       const char *parent, const char *name)
{
    ...
    assert_nonnull(baseband);
    object_property_add_child(OBJECT(t8103), name, OBJECT(baseband));

#if 1
    child = apple_dt_get_node(t8103->device_tree, "baseband");
    assert_nonnull(child);
    prop = apple_dt_get_prop(child, "interrupts");
    assert_nonnull(prop);
    ints = (uint32_t *)prop->data;
...
static void t8103_create_smc(AppleT8103MachineState *t8103)
{
    uint32_t i;
    uint32_t *ints;
    AppleDTProp *prop;
    ...
    object_property_add_child(OBJECT(t8103), "smc", OBJECT(smc));
    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    reg = (uint64_t *)prop->data;

    for (i = 0; i < prop->len / 16; ++i) {
        sysbus_mmio_map(smc, i, t8103->armio_base + reg[i * 2]);
    }
...
static void t8103_create_sio(AppleT8103MachineState *t8103)
{
    uint32_t i;
    uint32_t *ints;
    AppleDTProp *prop;
    ...
    // t8103.c
    sio = apple_sio_from_node(child, APPLE_A7IOP_V4, t8103->sio_protocol,
                              24000000);
    object_property_add_child(OBJECT(t8103), "sio", OBJECT(sio));
    ...
    for (i = 0; i < 2; ++i) {
        sysbus_mmio_map(sio, i, t8103->armio_base + reg[i * 2]);
    }
...
static void t8103_create_roswell(AppleT8103MachineState *t8103)
{
    AppleDTNode *child;
    AppleDTProp *prop;
    AppleI2CState *i2c;

    child = apple_dt_get_node(t8103->device_tree, "arm-io/i2c3/roswell");
    assert_nonnull(child);

    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    i2c = APPLE_I2C(
        object_property_get_link(OBJECT(t8103), "i2c3", &error_fatal));
    i2c_slave_create_simple(i2c->bus, TYPE_APPLE_ROSWELL,
                            *(uint8_t *)prop->data);
}

static void t8103_create_display_pmu(AppleT8103MachineState *t8103)
{
    AppleDTNode *child;
    AppleDTProp *prop;
    AppleI2CState *i2c;

    child = apple_dt_get_node(t8103->device_tree, "arm-io/i2c2/display-pmu");
    assert_nonnull(child);

    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    i2c = APPLE_I2C(
        object_property_get_link(OBJECT(t8103), "i2c2", &error_fatal));
    i2c_slave_create_simple(i2c->bus, TYPE_APPLE_CHESTNUT,
                            *(uint8_t *)prop->data);
}

static void t8103_create_pcie(AppleT8103MachineState *t8103)
{
    uint32_t i;
    uint32_t *ints;
    AppleDTProp *prop;
    ...
    prop = apple_dt_get_prop(apple_dt_get_node(t8103->device_tree, "chosen"),
                             "chip-id");
    assert_nonnull(prop);
    ...
    // TODO: S8000 needs it, and T8103 probably does need it as well.
    apple_dt_set_prop_null(child, "apcie-phy-tunables");
    // do not use no-phy-power-gating for T8103
    //// apple_dt_set_prop_null(child, "no-phy-power-gating");

    pcie = apple_pcie_from_node(child, chip_id);
    assert_nonnull(pcie);
    object_property_add_child(OBJECT(t8103), "apcie", OBJECT(pcie));
...
#define PORT_ENTRIES 4
    ...
    for (i = 0; i < msi_vectors; ++i) {
        sysbus_connect_irq(
            ...
            qdev_get_gpio_in(DEVICE(t8103->aic), msi_vector_offset + i));
    }

    sysbus_realize_and_unref(pcie, &error_fatal);
}

static void t8103_create_lm_backlight(AppleT8103MachineState *t8103)
{
    AppleDTNode *child;
    AppleDTProp *prop;
    AppleI2CState *i2c;

    child = apple_dt_get_node(t8103->device_tree, "arm-io/i2c0/lm3539");
    assert_nonnull(child);

    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    i2c = APPLE_I2C(
        object_property_get_link(OBJECT(t8103), "i2c0", &error_fatal));
    i2c_slave_create_simple(i2c->bus, TYPE_APPLE_LM_BACKLIGHT,
                            *(uint8_t *)prop->data);

    child = apple_dt_get_node(t8103->device_tree, "arm-io/i2c2/lm3539-1");
    assert_nonnull(child);
...
static void t8103_create_accbuck(AppleT8103MachineState *t8103)
{
    AppleDTNode *child;
    AppleDTProp *prop;
    AppleI2CState *i2c;

    child = apple_dt_get_node(t8103->device_tree, "arm-io/smc-i2c1/accbuck");
    assert_nonnull(child);

    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    i2c = APPLE_I2C(
        object_property_get_link(OBJECT(t8103), "smc-i2c1", &error_fatal));
    i2c_slave_create_simple(i2c->bus, TYPE_FAN53740, *(uint8_t *)prop->data);
}

static void t8103_create_misc(AppleT8103MachineState *t8103)
{
    AppleDTNode *armio;
    AppleDTNode *chosen;
    AppleDTNode *child;

    armio = apple_dt_get_node(t8103->device_tree, "arm-io");
    assert_nonnull(armio);
    chosen = apple_dt_get_node(t8103->device_tree, "chosen");
...
static void t8103_create_display(AppleT8103MachineState *t8103)
{
    MachineState *machine;
    SysBusDevice *sbd;
    AppleDTNode *child;
    ...
    AppleDARTState *dart = APPLE_DART(
        object_property_get_link(OBJECT(t8103), "dart-disp0", &error_fatal));
    assert_nonnull(dart);
    child =
        apple_dt_get_node(t8103->device_tree, "arm-io/dart-disp0/mapper-disp0");
    assert_nonnull(child);
    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);

    child = apple_dt_get_node(t8103->device_tree, "arm-io/disp0");

    sbd = adp_v4_from_node(
        child, MEMORY_REGION(apple_dart_iommu_mr(dart, ldl_le_p(prop->data))));

    qdev_prop_set_uint32(DEVICE(sbd), "width", t8103->disp_width);
    qdev_prop_set_uint32(DEVICE(sbd), "height", t8103->disp_height);

    t8103->video_args.row_bytes = t8103->disp_width * sizeof(uint32_t);
    t8103->video_args.width = t8103->disp_width;
    t8103->video_args.height = t8103->disp_height;
    t8103->video_args.depth.depth = sizeof(uint32_t) * 8;
    t8103->video_args.depth.rotate = 1;

    t8103->video_args.display =
    ...
    sysbus_mmio_map(sbd, 0, t8103->armio_base + reg[0]);
    ...
    object_property_add_child(OBJECT(t8103), "disp0", OBJECT(sbd));

    sysbus_realize_and_unref(sbd, &error_fatal);
}

static void t8103_create_scaler(AppleT8103MachineState *t8103)
{
    SysBusDevice *sbd;
    AppleDTNode *child;
    uint64_t *reg;
    ...
    AppleDARTState *dart = APPLE_DART(
        object_property_get_link(OBJECT(t8103), "dart-scaler", &error_fatal));
    g_assert_nonnull(dart);
    child = apple_dt_get_node(t8103->device_tree,
                              "arm-io/dart-scaler/mapper-scaler");
    g_assert_nonnull(child);
    prop = apple_dt_get_prop(child, "reg");
    g_assert_nonnull(prop);

    child = apple_dt_get_node(t8103->device_tree, "arm-io/scaler0");
    ...
    sysbus_mmio_map(sbd, 0, t8103->armio_base + reg[0]);
    sysbus_mmio_map(sbd, 1, t8103->armio_base + reg[2]);
    ...
    object_property_add_child(OBJECT(t8103), "scaler0", OBJECT(sbd));

    sysbus_realize_and_unref(sbd, &error_fatal);
}

static void t8103_create_sep(AppleT8103MachineState *t8103)
{
    AppleDTNode *armio;
    AppleDTNode *child;
    AppleSEPState *sep;
    ...
    dart = APPLE_DART(
        object_property_get_link(OBJECT(t8103), "dart-sep", &error_fatal));
...
static void t8103_create_sep_sim(AppleT8103MachineState *t8103)
{
    AppleDTNode *armio;
    AppleDTNode *child;
    ...
    object_property_add_child(OBJECT(t8103), "sep-sim", OBJECT(sep));

    prop = apple_dt_get_prop(child, "reg");
    assert_nonnull(prop);
    reg = (uint64_t *)prop->data;
    sysbus_mmio_map(SYS_BUS_DEVICE(sep), 0, t8103->armio_base + reg[0]);
...
static void t8103_create_mt_spi(AppleT8103MachineState *t8103)
{
    AppleSPIState *spi;
    DeviceState *device;
    DeviceState *gpio;
    ...
    spi = APPLE_SPI(
        object_property_get_link(OBJECT(t8103), "spi1", &error_fatal));
    // device = ssi_create_peripheral(apple_spi_get_bus(spi),
    // TYPE_APPLE_MT_SPI);
    device = qdev_new(TYPE_APPLE_MT_SPI);
    qdev_prop_set_uint32(device, "display_width", t8103->disp_width);
    qdev_prop_set_uint32(device, "display_height", t8103->disp_height);
    ssi_realize_and_unref(device, apple_spi_get_bus(spi), &error_fatal);

    gpio = DEVICE(
        object_property_get_link(OBJECT(t8103), "aop-gpio", &error_fatal));

    child = apple_dt_get_node(t8103->device_tree, "arm-io/spi1/multi-touch");
    assert_nonnull(child);
...
static void t8103_create_aop(AppleT8103MachineState *t8103)
{
    uint32_t i;
    uint32_t *ints;
    AppleDTProp *prop;
    ...
    object_property_add_child(OBJECT(t8103), "aop", OBJECT(aop));
    ...
    for (i = 0; i < 2; ++i) {
        sysbus_mmio_map(aop, i, t8103->armio_base + reg[i * 2]);
    }
    ...
    dart = APPLE_DART(
        object_property_get_link(OBJECT(t8103), "dart-aop", &error_fatal));
    assert_nonnull(dart);
...
static void t8103_create_mca(AppleT8103MachineState *t8103)
{
    AppleDTNode *child;
    AppleSIOState *sio;
    // DTBNode *mca5;
    ...
    child = apple_dt_get_node(t8103->device_tree, "arm-io/mca-switch");
    assert_nonnull(child);

    sio =
        APPLE_SIO(object_property_get_link(OBJECT(t8103), "sio", &error_fatal));
    ...
    object_property_add_child(OBJECT(t8103), "mca", OBJECT(sbd));
    sysbus_realize_and_unref(sbd, &error_fatal);
}

static void t8103_create_speaker_top(AppleT8103MachineState *t8103)
{
    AppleDTNode *child;
    AppleDTProp *prop;
    AppleI2CState *i2c;
    ...
    child =
        apple_dt_get_node(t8103->device_tree, "arm-io/i2c2/audio-speaker-top");
    assert_nonnull(child);
...
static void t8103_create_speaker_bottom(AppleT8103MachineState *t8103)
{
    AppleSPIState *spi;

    spi = APPLE_SPI(
        object_property_get_link(OBJECT(t8103), "spi3", &error_fatal));
    ssi_create_peripheral(apple_spi_get_bus(spi), TYPE_APPLE_CS42L77);
}

static void t8103_create_buttons(AppleT8103MachineState *t8103)
{
    SysBusDevice *buttons;
    AppleDTNode *child;

    child = apple_dt_get_node(t8103->device_tree, "buttons");
    assert_nonnull(child);

    buttons = apple_buttons_create(child);
    object_property_add_child(OBJECT(t8103), "buttons", OBJECT(buttons));
    sysbus_realize_and_unref(buttons, &error_fatal);
}

static void t8103_create_mipi_dsim(AppleT8103MachineState *t8103)
{
    int i;
    uint32_t *ints;
    AppleDTProp *prop;
    ...
    child = apple_dt_get_node(t8103->device_tree, "arm-io/mipi-dsim");
    assert_nonnull(child);

    sbd = synopsys_mipi_dsim_create(child);
    assert_nonnull(sbd);

    object_property_add_child(OBJECT(t8103), "mipi-dsim", OBJECT(sbd));
...
static void t8103_create_tempsensor(AppleT8103MachineState *t8103,
                                    const char *name, bool conflicts_pmgr)
{
    ...
    object_property_add_child(OBJECT(t8103), name, OBJECT(sbd));
    ...
    sysbus_connect_irq(
        sbd, 0, qdev_get_gpio_in(DEVICE(t8103->aic), ldl_le_p(prop->data)));

    sysbus_realize_and_unref(sbd, &error_fatal);
}

static void t8103_create_mtr_tempsensor(AppleT8103MachineState *t8103,
                                        const char *name)
{
    AppleDTNode *child;
    AppleDTProp *prop;
    ...
    create_unimplemented_device(name, t8103->armio_base + reg[0], reg[1]);
}

static void t8103_create_mic_temp_sensor2(AppleT8103MachineState *t8103)
{
    AppleDTNode *child;
    AppleDTProp *prop;
    AppleI2CState *i2c;

    child = apple_dt_get_node(t8103->device_tree, "arm-io/i2c1/mic-temp-sens2");
    assert_nonnull(child);
    ...
    i2c = APPLE_I2C(
        object_property_get_link(OBJECT(t8103), "i2c1", &error_fatal));
...
static void t8103_cpu_reset(AppleT8103MachineState *t8103)
{
    CPUState *cpu;
    ARMCPU *arm_cpu;
    AppleM1State *acpu;
    ...
    CPU_FOREACH (cpu) {
        ...
        acpu = container_of(arm_cpu, AppleM1State, parent_obj);

        arm_cpu->m_key_lo = m_lo;
        arm_cpu->m_key_hi = m_hi;

        if (t8103->securerom_filename == NULL) {
            if (acpu->cpu_id != M1_MAX_CPU) {
                object_property_set_uint(
                    OBJECT(cpu), "rvbar",
                    t8103->boot_info.kern_entry & ~0xFFFULL, &error_abort);
                cpu_reset(cpu);
            }
            if (acpu->cpu_id == 0) {
                arm_set_cpu_on(arm_cpu->mp_affinity,
                               t8103->boot_info.kern_entry,
                               t8103->boot_info.kern_boot_args_addr, 1, true);
            }
        } else {
            if (acpu->cpu_id != M1_MAX_CPU) {
...
static void t8103_reset(MachineState *machine, ResetType type)
{
    AppleT8103MachineState *t8103 = APPLE_T8103(machine);
    DeviceState *gpio;

    if (!runstate_check(RUN_STATE_RESTORE_VM)) {
        memset(t8103->pmgr_reg, 0, sizeof(t8103->pmgr_reg));
        ...
        if (!runstate_check(RUN_STATE_PRELAUNCH)) {
            t8103_memory_setup(t8103);
        }

        t8103_cpu_reset(t8103);
    }


    gpio =
        DEVICE(object_property_get_link(OBJECT(t8103), "gpio", &error_fatal));
    qemu_set_irq(qdev_get_gpio_in(gpio, GPIO_FORCE_DFU), t8103->force_dfu);
}

static void t8103_init_done(Notifier *notifier, void *data)
{
    AppleT8103MachineState *t8103 =
        container_of(notifier, AppleT8103MachineState, init_done_notifier);
    t8103_memory_setup(t8103);
    t8103_cpu_reset(t8103);
}
...
static void t8103_init(MachineState *machine)
{
    ...
    t8103 = APPLE_T8103(machine);

    if ((t8103->sep_fw_filename == NULL) != (t8103->sep_rom_filename == NULL)) {
    ...
    if (t8103->sep_fw_filename == NULL) {
        if (machine->smp.cpus > M1_MAX_CPU) {
            error_setg(&error_fatal,
    ...
    if (t8103->sep_rom_filename != NULL) {
    ...
    t8103->device_tree = apple_boot_load_dt_file(machine->dtb);
    if (t8103->device_tree == NULL) {
        error_setg(&error_fatal, "Failed to load device tree");
        return;
    }

    t8103->kernel = apple_boot_load_kernel(machine->kernel_filename, NULL);
    if (t8103->kernel == NULL) {
        error_setg(&error_fatal, "Failed to load kernel");
        return;
    }

    build_version = apple_boot_build_version(t8103->kernel);
    info_report("%s %u.%u.%u", apple_boot_platform_string(t8103->kernel),
    ...
    if (t8103->securerom_filename == NULL) {
        apple_boot_get_kc_bounds(t8103->kernel, NULL, &g_virt_base, &kc_end,
                                 NULL, NULL);
        info_report("Kernel virtual low: 0x%016" VADDR_PRIx, g_virt_base);
        info_report("Kernel virtual high: 0x%016" VADDR_PRIx, kc_end);

        g_phys_base = (hwaddr)apple_boot_get_macho_buffer(t8103->kernel);

        t8103_patch_kernel(t8103->kernel, build_version);

        t8103->trustcache = apple_boot_load_trustcache_file(
            t8103->trustcache_filename, &t8103->boot_info.trustcache_size);

        if (t8103->ticket_filename != NULL) {
            if (!g_file_get_contents(t8103->ticket_filename,
                                     &t8103->boot_info.ticket_data,
                                     &t8103->boot_info.ticket_length, NULL)) {
                error_setg(&error_fatal, "Failed to read ticket from `%s`",
                           t8103->ticket_filename);
                return;
            }
        }
    } else {
        if (!g_file_get_contents(t8103->securerom_filename, &t8103->securerom,
                                 &t8103->securerom_size, NULL)) {
            error_setg(&error_fatal, "Failed to load SecureROM from `%s`",
                       t8103->securerom_filename);
            return;
        }
    }

    apple_dt_set_prop_u32(t8103->device_tree, "clock-frequency", 24000000);
    child = apple_dt_get_node(t8103->device_tree, "arm-io");
    assert_nonnull(child);

    t8103->chip_revision = 0x20;
    apple_dt_set_prop_u32(child, "chip-revision", t8103->chip_revision);

    apple_dt_set_prop(child, "clock-frequencies",
                      sizeof(t8103_clock_frequencies), t8103_clock_frequencies);
    apple_dt_set_prop(child, "clock-frequencies-nclk",
                      sizeof(t8103_clock_frequencies_nclk),
                      t8103_clock_frequencies_nclk);

    prop = apple_dt_get_prop(child, "ranges");
    assert_nonnull(prop);

    ranges = (hwaddr *)prop->data;
    t8103->armio_base = ranges[1];
    t8103->armio_size = ranges[2];

    apple_dt_set_prop_strn(t8103->device_tree, "platform-name", 32, "t8103");
    apple_dt_set_prop_strn(t8103->device_tree, "model-number", 32,
                           t8103->model_number);
    apple_dt_set_prop_strn(t8103->device_tree, "region-info", 32,
                           t8103->region_info);
    apple_dt_set_prop_strn(t8103->device_tree, "config-number", 64,
                           t8103->config_number);
    apple_dt_set_prop_strn(t8103->device_tree, "serial-number", 32,
                           t8103->serial_number);
    apple_dt_set_prop_strn(t8103->device_tree, "mlb-serial-number", 32,
                           t8103->mlb_serial_number);
    apple_dt_set_prop_strn(t8103->device_tree, "regulatory-model-number", 32,
    ...
    apple_dt_set_prop_u64(child, "unique-chip-id", t8103->ecid);

    // Update the display parameters
    apple_dt_set_prop_u32(child, "display-scale", 2);

    child = apple_dt_get_node(t8103->device_tree, "product");
    ...
    for (uint32_t i = 0; i < NUM_UARTS; ++i) {
        t8103_create_s3c_uart(t8103, i, serial_hd(i));
    }

    t8103_pmgr_setup(t8103);
    t8103_amcc_setup(t8103);
    t8103_create_gpio(t8103, "gpio");
    t8103_create_gpio(t8103, "smc-gpio");
    t8103_create_gpio(t8103, "nub-gpio");
    t8103_create_gpio(t8103, "aop-gpio");
    t8103_create_i2c(t8103, "i2c0");
    t8103_create_i2c(t8103, "i2c1");
    t8103_create_i2c(t8103, "i2c2");
    t8103_create_i2c(t8103, "i2c3");
    t8103_create_i2c(t8103, "smc-i2c0");
    t8103_create_i2c(t8103, "smc-i2c1");
    t8103_create_dart(t8103, "dart-usb", false);
    t8103_create_dart(t8103, "dart-sio", false);
    t8103_create_dart(t8103, "dart-disp0", false);
    t8103_create_dart(t8103, "dart-sep", false);
    t8103_create_dart(t8103, "dart-apcie2", true);
    t8103_create_dart(t8103, "dart-apcie3", true);
    t8103_create_dart(t8103, "dart-aop", false);
    t8103_create_dart(t8103, "dart-rsm", false);
    t8103_create_dart(t8103, "dart-scaler", false);
    t8103_create_pcie(t8103);
    t8103_create_ans(t8103);
    t8103_create_usb(t8103);
    t8103_create_wdt(t8103);
    t8103_create_aes(t8103);
    t8103_create_spmi(t8103, "spmi0");
    t8103_create_spmi(t8103, "spmi1");
    t8103_create_spmi(t8103, "spmi2");
    t8103_create_pmu(t8103, "spmi0", "spmi-pmu");
    t8103_create_smc(t8103);
#ifdef ENABLE_BASEBAND
    t8103_create_baseband_spmi(t8103, "spmi1", "baseband-spmi");
    t8103_create_baseband(t8103);
#endif
    t8103_create_sio(t8103);
    t8103_create_spi0(t8103);
    t8103_create_spi(t8103, 1);
    t8103_create_spi(t8103, 3);

    if (t8103->sep_rom_filename && t8103->sep_fw_filename) {
        t8103_create_sep(t8103);
    } else {
        ...
        return;
#else
        t8103_create_sep_sim(t8103);
#endif
    }

    t8103_create_roswell(t8103);
    t8103_create_lm_backlight(t8103);
    t8103_create_display_pmu(t8103);
    t8103_create_display(t8103);
    t8103_create_mt_spi(t8103);
    t8103_create_aop(t8103);
    t8103_create_mca(t8103);
    t8103_create_speaker_top(t8103);
    t8103_create_speaker_bottom(t8103);
    t8103_create_buttons(t8103);
    t8103_create_mipi_dsim(t8103);
    t8103_create_scaler(t8103);
    t8103_create_accbuck(t8103);
    t8103_create_misc(t8103);

    t8103_create_tempsensor(t8103, "tempsensor0", false);
    t8103_create_tempsensor(t8103, "tempsensor1", false);
    t8103_create_tempsensor(t8103, "tempsensor2", false);
    t8103_create_tempsensor(t8103, "tempsensor3", false);
    t8103_create_tempsensor(t8103, "tempsensor4", false);
    t8103_create_tempsensor(t8103, "tempsensor5", false);
    t8103_create_tempsensor(t8103, "mtrtempsensor6", true);
    t8103_create_tempsensor(t8103, "mtrtempsensor7", true);
    t8103_create_tempsensor(t8103, "mtrtempsensor8", true);
    t8103_create_tempsensor(t8103, "mtrtempsensor11", false);
    t8103_create_tempsensor(t8103, "mtrtempsensor13", false);
    t8103_create_mtr_tempsensor(t8103, "mtrtempsensor14");
    t8103_create_mtr_tempsensor(t8103, "mtrtempsensor15");
    t8103_create_mic_temp_sensor2(t8103);

    t8103->init_done_notifier.notify = t8103_init_done;
    qemu_add_machine_init_done_notifier(&t8103->init_done_notifier);
}

static ram_addr_t t8103_fixup_ram_size(ram_addr_t size)
{
...
static void t8103_set_boot_mode(Object *obj, const char *value, Error **errp)
{
    ...
    APPLE_T8103(obj)->boot_info.boot_mode = boot_mode;
}

static char *t8103_get_boot_mode(Object *obj, Error **errp)
{
    switch (APPLE_T8103(obj)->boot_info.boot_mode) {
    case kAppleBootModeEnterRecovery:
...
static void t8103_class_init(ObjectClass *klass, const void *data)
{
    MachineClass *mc = MACHINE_CLASS(klass);
    ObjectProperty *oprop;

    mc->desc = "Apple T8103 SoC (Mac M1)";
    mc->init = t8103_init;
    mc->reset = t8103_reset;
    mc->max_cpus = M1_MAX_CPU + 1;
    mc->auto_create_sdcard = false;
    mc->no_floppy = true;
    mc->no_cdrom = true;
    mc->no_parallel = true;
    mc->default_cpu_type = TYPE_APPLE_M1;
    mc->minimum_page_bits = 14;
    mc->default_ram_size = 4 * GiB;
    mc->fixup_ram_size = t8103_fixup_ram_size;
    mc->default_ram_id = "t8103.ram";

    object_class_property_add_str(klass, "trustcache",
                                  t8103_get_trustcache_filename,
                                  t8103_set_trustcache_filename);
    object_class_property_set_description(klass, "trustcache", "TrustCache");
    object_class_property_add_str(klass, "ticket", t8103_get_ticket_filename,
                                  t8103_set_ticket_filename);
    object_class_property_set_description(klass, "ticket", "AP Ticket");
    object_class_property_add_str(klass, "sep-rom", t8103_get_sep_rom_filename,
                                  t8103_set_sep_rom_filename);
    object_class_property_set_description(klass, "sep-rom", "SEP ROM");
    object_class_property_add_str(klass, "sep-fw", t8103_get_sep_fw_filename,
                                  t8103_set_sep_fw_filename);
    object_class_property_set_description(klass, "sep-fw", "SEP Firmware");
    object_class_property_add_str(klass, "securerom",
                                  t8103_get_securerom_filename,
                                  t8103_set_securerom_filename);
    object_class_property_set_description(klass, "securerom", "SecureROM");
    oprop = object_class_property_add_str(
        klass, "boot-mode", t8103_get_boot_mode, t8103_set_boot_mode);
    object_property_set_default_str(oprop, "auto");
    object_class_property_set_description(klass, "boot-mode", "Boot Mode");
    oprop = object_class_property_add(klass, "ecid", "uint64", t8103_get_ecid,
                                      t8103_set_ecid, NULL, NULL);
    object_property_set_default_uint(oprop, 0x1122334455667788);
    object_class_property_set_description(klass, "ecid", "Device ECID");
    object_class_property_add_bool(klass, "kaslr-off", t8103_get_kaslr_off,
                                   t8103_set_kaslr_off);
    object_class_property_set_description(klass, "kaslr-off", "Disable KASLR");
    object_class_property_add_bool(klass, "force-dfu", t8103_get_force_dfu,
                                   t8103_set_force_dfu);
    ...
    object_class_property_add_enum(
        ...
        &USBTCPRemoteConnType_lookup, t8103_get_usb_conn_type,
        t8103_set_usb_conn_type);
    object_class_property_set_description(klass, "usb-conn-type",
                                          "USB Connection Type");
    object_class_property_add_str(klass, "usb-conn-addr",
                                  t8103_get_usb_conn_addr,
                                  t8103_set_usb_conn_addr);
    ...
    object_class_property_add(klass, "usb-conn-port", "uint16",
                              t8103_get_usb_conn_port, t8103_set_usb_conn_port,
                              NULL, NULL);
    object_class_property_set_description(klass, "usb-conn-port",
                                          "USB Connection Port");
    oprop = object_class_property_add_str(
        klass, "model", t8103_get_model_number, t8103_set_model_number);
    object_property_set_default_str(oprop, "CKI12");
    object_class_property_set_description(klass, "model", "Model Number");
    oprop = object_class_property_add_str(
        klass, "region-info", t8103_get_region_info, t8103_set_region_info);
    ...
    oprop = object_class_property_add_str(klass, "config-number",
                                          t8103_get_config_number,
                                          t8103_set_config_number);
    ...
    oprop = object_class_property_add_str(klass, "serial-number",
                                          t8103_get_serial_number,
                                          t8103_set_serial_number);
    object_property_set_default_str(oprop, "INFERNO_1122");
    object_class_property_set_description(klass, "serial-number",
                                          "Serial Number");
    oprop = object_class_property_add_str(
        klass, "mlb", t8103_get_mlb_serial_number, t8103_set_mlb_serial_number);
    ...
    oprop = object_class_property_add_str(klass, "regulatory-model",
                                          t8103_get_regulatory_model,
                                          t8103_set_regulatory_model);
    ...
    oprop = object_class_property_add(klass, "disp-width", "uint32",
                                      t8103_get_disp_width,
                                      t8103_set_disp_width, NULL, NULL);
    object_property_set_default_uint(oprop, 828);
    oprop = object_class_property_add(klass, "disp-height", "uint32",
                                      t8103_get_disp_height,
                                      t8103_set_disp_height, NULL, NULL);
    object_property_set_default_uint(oprop, 1792);
}

static const TypeInfo t8103_info = {
    .name = TYPE_APPLE_T8103,
    .parent = TYPE_MACHINE,
    .instance_size = sizeof(AppleT8103MachineState),
    .class_size = sizeof(AppleT8103MachineClass),
    .class_init = t8103_class_init,
};

static void t8103_types(void)
{
    type_register_static(&t8103_info);
}

type_init(t8103_types)
