...
#include "hw/arm/apple-silicon/m1_gxf.h"
...
static const ARMCPRegInfo apple_m1_gxf_cp_override_reginfo[] = {
    {
        .name = "TPIDR_EL1",
...
static const ARMCPRegInfo apple_m1_gxf_cp_reginfo[] = {
    {
        .name = "GXF_CONFIG_EL1",
...
void apple_m1_init_gxf_override(AppleM1State *cpu)
{
    if (arm_feature(&cpu->parent_obj.env, ARM_FEATURE_GXF)) {
        define_arm_cp_regs(&cpu->parent_obj, apple_m1_gxf_cp_override_reginfo);
    }
}
...
{
    if (arm_feature(&cpu->parent_obj.env, ARM_FEATURE_GXF)) {
        define_arm_cp_regs(&cpu->parent_obj, apple_m1_gxf_cp_reginfo);
    }
}
